...
== On technical artefacts and concerns

It is difficult to draw a clear line between semantic and technical interoperability artefacts and concerns. This section aims at providing some guidelines and hints on how to distinguish between the two. The best way at look at them is in terms of concerns addressed within each layer, but this discussion resembles a lot the one from xref:references.adoc#ref:97[1975] of distinguishing "Conceptual Data Models", "Logical Data Models" and "Physical Data Models" in xref:references.adoc#ref:97[ANSI] .
