...
from one another, just as the OWL specification defines it [xref:references.adoc#ref:43[43]].
...
existing ones in the superclass(es). Also, make sure there are no logical contradictions [xref:gc-semantic-conventions.adoc[Semantic conventions]].
...
duplicate existing ones in the superclass(es). Make sure there are no logical contradictions [xref:gc-semantic-conventions.adoc[Semantic conventions]].
