...
The use of "standard" datatypes is strongly recommended. The creation of custom datatypes shall be avoided, whenever is possible.
...
(consisting of single literal value) and composite types (consisting of multiple attributes)
(see also [xref:references.adoc#ref:41[41, sec 3.1]]). In fact, the composite datatypes must be defined as classes and handled as such.
...
XSD [xref:references.adoc#ref:42[42]] and RDF [xref:references.adoc#ref:33[33]],
which cover the standard and most common types. Thus,
definitions of custom datatypes shall be avoided unless the model really needs them.
Such cases are, however, rare.

The datatypes defined in the UML model (and custom ones) are resolved into their
XSD equivalent using the correspondence table below. Note that the family of
...
for xsd:dateTime. Therefore, it is recommended to follow the OWL2 specification,
...
Moreover, it is recommended to use OWL 2 compliant XSD and RDF standard datatypes, if possible.
They may be useful in indicating a specific datatype which is not possible with
...
For the model consistency, it is recommended that the proxy datatypes be defined
in the model for the XSD and RDF datatypes used in the model. The proxies
