...
Indicators of deontic modalities for classes and properties do not have semantic or normative value, yet they may serve as editorial annotations.
|===
...
The main instrument for properties is usage of cardinality constraints per property per class.

For classes however, such indicators shall be avoided because the same class may be mandatory and optional in different instantiation scenariosfor the same application profile.

For example in DCAT-AP the class dcat:Dataset is mandatory ...

// what may have been implicationf on validation ...violation constraints ... move to the rule above